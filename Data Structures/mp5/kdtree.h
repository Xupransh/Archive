...
class KDTree
{
  private:
     ...
     *
     */
    KDTreeNode * KDTreeHelper(vector< Point<Dim> > &, int, int, int);
     ...
     *
     */
    int selectMedian(vector< Point<Dim> > &, int, int, int, int);

    int partition(vector<Point<Dim> > & newPoints,
                               int dimension, int low, int high, int pivot_index);

    double distance(const Point<Dim> & first,
                    const Point<Dim> & second) const;


    void copyTree(KDTreeNode *& curr, KDTreeNode *& other);


    Point<Dim> NearestNeighborRecurse(KDTreeNode * curr, int dimension,

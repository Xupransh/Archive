...
TEST_CASE("remove vertex" , "[weight = 1]") {
...
TEST_CASE("adjacent same v", "[weight = 1]") {
  auto g = createTestGraph();
  REQUIRE(g.isAdjacent("h", "h") == false);
}

TEST_CASE("delete edge", "[weight = 1]") {
  auto g = createTestGraph();

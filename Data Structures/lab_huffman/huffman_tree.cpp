                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (!singleQueue.empty()) {
        f_1 = singleQueue.front()->freq.getFrequency();
    } if (!mergeQueue.empty()) {
        f_2 =  mergeQueue.front()->freq.getFrequency();
    } if (f_1 <= f_2) {
...
bool HuffmanTree::quitCondition(queue<TreeNode*> singleQueue, queue<TreeNode*> mergeQueue) {
    if (singleQueue.empty() && mergeQueue.empty()) {
        return true;
    } if (singleQueue.empty() && mergeQueue.size() == 1) {
        return true;
    } else if (singleQueue.size() == 1 && mergeQueue.empty()) {
        return true;
    }
    return false;
}
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    } while (!quitCondition(singleQueue, mergeQueue)) {
        smaller_1 = removeSmallest(singleQueue, mergeQueue);
        smaller_2 = removeSmallest(singleQueue, mergeQueue);
        f_1 = smaller_1->freq.getFrequency();
        f_2 = smaller_2->freq.getFrequency();
        mergeQueue.push(new TreeNode(f_1+f_2));
        mergeQueue.back()->right = smaller_2;
        mergeQueue.back()->left = smaller_1;
    } if (!mergeQueue.empty()) {
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
         ...
         */
        if (!current) {
            ss << "Written file was incorrect!" << endl;
            return;

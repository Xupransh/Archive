...
size_t heap<T, Compare>::maxPriorityChild(size_t currentIdx) const
{
    ...
    if (rightChildIdx < _elems.size() && leftChildIdx < _elems.size()) {
        if (higherPriority(_elems[rightChildIdx], _elems[leftChildIdx])) {
            return rightChildIdx;
        }
        return leftChildIdx;
    } else if (rightChildIdx < _elems.size()) {
        return rightChildIdx;
    }
    return leftChildIdx;
}
...
void heap<T, Compare>::updateElem(const size_t & idx, const T& elem)
{
    ...
    T old_value = _elems[idx];
    _elems[idx] = elem;
    if (higherPriority(elem, old_value)) {
        heapifyUp(idx);
    } else {
        heapifyDown(idx);
    }

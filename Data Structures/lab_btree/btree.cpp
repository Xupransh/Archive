...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    std::vector<DataPair> el_temp(mid_elem_itr+1, new_left->elements.end());
    std::vector<BTreeNode*> ch_temp(mid_child_itr, new_left->children.end());

    new_right->elements = el_temp;
    new_right->children = ch_temp;

...
//resizes the id array of a vertex set to double the size
int32_t
...
storePathInfo(graph_t *g, heap_t *h, path_t *path, int32_t dest_index)
{
    ...
    int32_t *path_id_copy = malloc(n_vertices*sizeof(*path_id_copy));

    if(!path_id_copy)
        return 0;

    path->id = path_id_copy;
...
Sorting the vertex takes place after the function is called
*/
void
nodeRecurse (locale_t* loc, vertex_set_t* vs, pyr_tree_t* p, int32_t nnum)
{
    //resize array if out of size
    if(vs->count >= vs->id_array_size)
    {
        if(!resizeVertexSetID(vs))
            return;
    }
    ...
    //if part of loc lies in the 2st and 3th quadrant
    if(loc->x - loc->range <= p->node[nnum].x)
      {
          //if part of loc lies above y_left, explore 2nd quadrant of the pyramid
          if(loc->y + loc->range >= p->node[nnum].y_left)
          {
              if(4*nnum+3 >= p->n_nodes)
                  isLeafInRange(loc, p, vs, nnum);
              else
                  nodeRecurse(loc, vs, p, 4*nnum+3);
          }
...
static int
sort_by_id(const void* id_1, const void* id_2)
{
    const int32_t *id1 = id_1;
    const int32_t *id2 = id_2;
...
find_nodes (locale_t* loc, vertex_set_t* vs, pyr_tree_t* p, int32_t nnum)
{
    ...
    nodeRecurse(loc, vs, p, nnum);
    qsort(vs->id, vs->count, sizeof(*(vs->id)), sort_by_id);
}
/*
...
trim_nodes (graph_t* g, vertex_set_t* vs, locale_t* loc)
{
    ...
    while(count < vs->count && vs->count > 0)
    {
        if(!in_range(loc, (g->vertex[vs->id[count]]).x, (g->vertex[vs->id[count]]).y))
        {
            ...
            //reset the count to the current element
            count--;
        }
        count++;
    }
